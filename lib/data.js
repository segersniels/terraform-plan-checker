...
const generateOutput = (json, lines, type) => {
    return new Promise((resolve, reject) => {
        if (type === 'changes') {
            data.changes = lines;
            if (lines.length > 0) console.log('   Lines that were changed or added:');
        } else {
            data.deletes = lines;
            if (lines.length > 0) console.log('   Lines that were deleted:');
        }
        let changes = [];
...
exports.processDefinitions = async definitions => {
    if (definitions.clean) {
        ...
        .then(output => _.forEach(output, change => console.log(colors.yellow(change))));
