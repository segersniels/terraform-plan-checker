...
const removeUnwanted = lines => {
    return new Promise((resolve, reject) => {
        ...
        // We need this to check if small changes to a name don't show up in the deleted lines
        // They only need to be shown in the changed lines
        _.forEach(lines, (line, index) => {
            const array = _.trim(line).replace(/"/g,'').split(':');
            const lineKey = array.shift();
            const lineValue = _.trim(array.join(':'));
            _.forEach(changes, change => {
                if (change.indexOf(lineValue) > -1) similar.push(line);
    ...
    return new Promise((resolve, reject) => {
        ...
        _.forEach(lines, line => {
            ...
            _.forEach(latest, object => {
                ...
                _.forEach(keys, key => {
                    if (key === lineKey && _.toString(object[key]) === lineValue) {
                        changes.push(`    ${object.name}:  ${line}`);
                    }
                    _.forEach(object[key], sub => {
                        if (typeof sub[lineKey] !== 'undefined' && sub[lineKey] === lineValue) {
                            changes.push(`    ${object.name}:  ${line}`);
                        }
                    });
                });
