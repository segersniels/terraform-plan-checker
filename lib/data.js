...
exports.initDefinitions = (file, clean) => {
...
const removeUnwantedChangedLines = lines => {
    return new Promise((resolve, reject) => {
        ...
        _.forEach(lines, (line, index) => {
            if (!line.match(/(\[\]|essential|protocol|memory|image)/)) adjusted.push(line);
        });
...
const addTopLevelChanges = async output => {
    const oldest = JSON.parse(await grabContainerDefinition(data.file, 'oldest'));
    const latest = JSON.parse(await grabContainerDefinition(data.file, 'latest'));
    const file = ((typeof process.argv[2] !== 'undefined') ? process.argv[2] : "stdin");
        ...
        if (!_.includes(data.new, name)) {
            ...
            _.forEach(_.keys(late), key => {
                if (late[key] !== old[key] && typeof late[key] !== 'object') {
                    output.push(`    - ${file} | (${name})  "${key}": "${late[key]}"`);
                }
                if (typeof late[key] === 'object' && key !== 'environment') {
                    if (_.join(late[key], " ") !== _.join(old[key], " ")) {
                        output.push(`    - ${file} | (${name})  "${key}": "${_.join(late[key], ", ")}"`);
                    }
                }
            });
        }
