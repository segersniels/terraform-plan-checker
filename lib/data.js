    ...
    return new Promise((resolve, reject) => {
        fs.stat('./' + process.argv[2], err => {
            if (err) reject(err);
                ...
                exec(`terraform show ${process.argv[2]} |grep container_definitions`, (err, stdout, stderr) => {
                    if (stdout === "") {
                        console.error("ERR: this plan does nothing.");
                        process.exit();
                    }
                    const split = stdout.split('=>');
                    const object = type === 'oldest' ? split[0] : split[1];
                    const outReplaced = stripAnsi(object).trim()
                        .replace("container_definitions:", "")
...
exports.grabDefinitions = (file, clean) => {
    return new Promise(async (resolve, reject) => {
        ...
        if (data.oldest.length > data.latest.length) {
            const oldestNames = _.map(JSON.parse(data.oldest), i => i.name);
            const latestNames = _.map(JSON.parse(data.latest), i => i.name);
            let similarities = [];
            _.forEach(oldestNames, (oldName, index) => {
                ...
                if (similarities.indexOf(1) < 0) data.new.push(oldName);
                similarities = [];
            });
        }
