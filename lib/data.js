...
exports.grabDefinitions = clean => {
    return new Promise(async (resolve, reject) => {
        ...
        if (data.oldest.length > data.latest.length) {
            const oldest = _.map(JSON.parse(data.oldest), i => i.name);
            const latest = _.map(JSON.parse(data.latest), i => i.name);
            _.forEach(oldest, (name, index) => {
                const similarity = ((typeof latest[index] !== 'undefined') ? stringSimilarity.compareTwoStrings(name, latest[index]) : 0);
                if (similarity < 0.5) data.new.push(name);
            });
        }
